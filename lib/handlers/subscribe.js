...
function storeSubscriptions (sub, done) {
  ...
  // ---------------------------------------------------------------------------------------------------
  // Find index of topic after authorize sub and addSubscription.
  // Change: addSubscription for each topic separately otherwise the modified topic is not being stored
  // Sub obj: {topic: 'topic', qos: 1}
  // ---------------------------------------------------------------------------------------------------
  var index = packet.subscriptions.findIndex(obj => obj.topic === sub.topic)
  ...
  if (sub && index === -1) {  // Changed: exit if topic is not found in subscription array
  ...
  perst.addSubscriptions(client, [packet.subscriptions[index]], function (err) {  // Changed: Subscribe topics individually after AuthSub
    done(err, sub)
  })
}
